...
class Jira {
  ...
  display(events) {
    ...
    for (let e of events) {
      const line = [
        moment(e.start).format('MM-DD (ddd)'), `${moment.duration(e.duration, 'minutes').asHours()}h`, e.summary || e.description,
      ].join("\t");
    ...
    console.log(colors.bold(`Total duration: ${totalDuration.format('HH:mm')}`));
  }
