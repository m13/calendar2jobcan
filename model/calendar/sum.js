...
function groupByDate(iterator, current) {
  ...
  if (iterator[current.date].duration < 60 * 4 && iterator[current.date].duration + current.duration >= 60 * 4) {
    iterator[current.date].extra = 60;
    iterator[current.date].breaktime = '01:00';
  }
  iterator[current.date].duration += current.duration;
  iterator[current.date].clockout = moment(`2000-01-01 ${iterator[current.date].clockin}`)
    .add(iterator[current.date].extra + iterator[current.date].duration, 'minutes').format('HH:mm');
  iterator[current.date].events += 1;
  return iterator;
}

module.exports = function (events) {
  return events.map(asCompactedEvent)
